...
`RESTIC_PASSWORD`, `B2_ACCOUNT_ID`/`B2_ACCOUNT_KEY`, the four `HEALTHCHECKS_*_PING_URL` values. There is
no `.env.<scope>` file scheme to maintain for Netlify — that five-file split (preserved below, in
"Environment (VPS)") existed specifically to keep VPS containers from seeing credentials they had no
use for; Netlify's own environment-variable store replaces it, with far fewer distinct credentials to
begin with.
...
A dump that was never restored is not a proven backup — this is the same reasoning the old
`scripts/verify.sh` was built around. The procedure below was **actually run**, once, against the local
