...
type TabValue = 'details' | 'progress' | 'notes' | 'trophies';

/**
 * The full per-game edit experience — everything that used to live in
 * `GameDialog`'s edit path, moved here so it lives on its own page instead
 * of a modal, plus a live-fetched Trophies section. `GameDialog`
...
export function GamePage({ game }: { readonly game: Game }): React.ReactElement {
  ...
  // Controlled (unlike the create-only dialog's uncontrolled `defaultValue`)
  // specifically so the Trophies tab's FIRST activation can be observed and
  // trigger the live fetch — see `onValueChange` below. Same one-shot ref
  // idiom `titleEditedRef`/`suppressNextSearchRef` above already use: the
  // fetch fires once, never again on repeat tab-switches.
  ...
  function handleTabChange(value: string): void {
    ...
    if (value !== 'trophies' || trophyFetchStartedRef.current) return;
    trophyFetchStartedRef.current = true;
    setTrophyState({ status: 'loading' });
    startTransition(async () => {
      const result = await fetchGameTrophiesAction(game.id);
      setTrophyState(result.ok ? { status: 'loaded', trophies: result.trophies } : { status: 'failed', reason: result.reason });
    });
  }
  ...
  return (
    <>
      ...
      <div className="mx-auto mt-6 max-w-2xl">
        <form id="game-form" action={submit} className="space-y-4">
           ...
           * default (unmount the inactive panel) — same reason a full page
           * doesn't change: every field is a native named input/hidden-input
           * that reaches the server ONLY because it is present in the DOM at
           * submit time, and `game-actions.ts`'s `parse()` treats an ABSENT
           * key on update as "explicitly cleared." Controlled (`value`/
           * `onValueChange`), unlike the create dialog's uncontrolled
           * `defaultValue`, so the Trophies tab's first activation can
           * trigger its live fetch.
           */}
          <Tabs value={activeTab} onValueChange={handleTabChange}>
            <TabsList>
              ...
              {hasTrophies ? <TabsTrigger value="trophies">Trophies</TabsTrigger> : null}
            </TabsList>
            ...
            <TabsContent value="progress" forceMount className="pt-4 data-[state=inactive]:hidden">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="flex items-center gap-2">
                  <Checkbox id="platinum" name="platinum" value="true" defaultChecked={game.platinum} />
            ...
            {hasTrophies ? (
              <TabsContent value="trophies" forceMount className="pt-4 data-[state=inactive]:hidden">
